...
public class MainAgent extends Agent {
    ...
    private class GameManager extends SimpleBehaviour {
        ...
        void playGame(ArrayList<PlayerInformation> players, int num_partida){
            Partida p = new Partida(players);
            //int[] plays = new int[players.size()];
            
            ACLMessage msg = new ACLMessage(ACLMessage.INFORM);
            msg.setContent(getNewGameString(players));
            for(PlayerInformation player : players){
            ...
            for(int g = 0; g < parameters.Generaciones; g ++){
                if(g==0){
                    ...
                    Collections.sort(players, new Comparator<PlayerInformation>() {
                        @Override
                        public int compare(MainAgent.PlayerInformation o1, MainAgent.PlayerInformation o2) {
                            if(o1.num_partidas > o2.num_partidas){
                                return 1;
                            }else if(o1.num_partidas < o2.num_partidas){
                                return -1;
                            }else{
                                return 0;
                            }
                ...
                if(players.size() > parameters.N){
                    ...
                    gui.logLine("Ha eliminado al jugador " + players.get(players.size()-1).aid.getName());
    ...
    public void pinta_Resultados(){
        String[] column_names = {"Jugador", "Puntuación", "Avg.Puntuación"};
        ...
        for(int i = 0; i < playerAgents.size(); i ++){
            ...
            gui.model.addRow(new Object[]{agent_name, Integer.toString(p_info.puntuacion),Float.toString(p_info.avg_puntuacion)});
        }
    }

    String getNewGameString(ArrayList<PlayerInformation> players){
    ...
    class PlayerInformation implements Comparable<PlayerInformation>{
        ...
        void restart_player_values(){
            this.endowment = parameters.E;
            this.inversion = 0;
        }

        void calcula_avg_puntuacion(){
